...
template<int SPLIT>
__device__ __forceinline__ void process_work(const uint4*codes,const std::uint8_t*scales,const std::uint16_t*x,float*partials,std::uint32_t*counters,std::uint16_t*y,std::uint32_t n,std::uint32_t k,std::uint32_t work,std::uint32_t*arrived){
...
#pragma unroll
 ...
 for(std::uint32_t nr=lane;nr<16U;nr+=32U){const std::uint32_t source=(nr&7U)*4U,reg=(nr>>3U)*2U;float sum=0;
#pragma unroll
...
__global__ __launch_bounds__(kThreads,6) void scheduler_kernel(
 ...
 const std::uint16_t*input,const std::uint16_t*attention,float*pqa,float*pqb,float*pkv,float*poa,float*pob,std::uint32_t*cqa,std::uint32_t*cqb,std::uint32_t*ckv,std::uint32_t*coa,std::uint32_t*cob,
 std::uint16_t*yqa,std::uint16_t*yqb,std::uint16_t*ykv,std::uint16_t*yoa,std::uint16_t*yob,std::uint32_t*barrier){
 __shared__ std::uint32_t arrived[kWarps];const std::uint32_t pw=blockIdx.x*kWarps+(threadIdx.x>>5U),stride=gridDim.x*kWarps;
 for(std::uint32_t w=pw;w<(1024U/16U)*16U;w+=stride)process_work<16>(qa,qas,input,pqa,cqa,yqa,1024,4096,w,arrived);
 grid_barrier(barrier,barrier+1);
 constexpr std::uint32_t qbwork=(40960U/16U),kvwork=(512U/16U)*16U;
 for(std::uint32_t w=pw;w<qbwork+kvwork;w+=stride){if(w<qbwork)process_work<1>(qb,qbs,yqa,pqb,cqb,yqb,40960,1024,w,arrived);else process_work<16>(kv,kvs,input,pkv,ckv,ykv,512,4096,w-qbwork,arrived);}
 grid_barrier(barrier,barrier+1);
 for(std::uint32_t w=pw;w<(8192U/16U)*4U;w+=stride)process_work<4>(oa,oas,attention,poa,coa,yoa,8192,4096,w,arrived);
 grid_barrier(barrier,barrier+1);
 for(std::uint32_t w=pw;w<(4096U/16U)*8U;w+=stride)process_work<8>(ob,obs,yoa,pob,cob,yob,4096,8192,w,arrived);
}
...
Error oracle(const HostMatrix&w,const std::vector<std::uint16_t>&x,const std::vector<std::uint16_t>&y){Error r;for(std::uint32_t n=0;n<w.n;++n){double sum=0,ab=0;for(std::uint32_t k=0;k<w.k;++k){const auto s=w.scales[(n/128U)*(w.k/128U)+k/128U];const double p=double(bf16_value(bf16_bits(e4(w.canon[std::size_t(n)*w.k+k])*e8(s))))*bf16_value(x[k]);sum+=p;ab+=std::abs(p);}const auto expected=bf16_bits(float(sum));if(y[n]!=expected)++r.bad;r.norm=std::max(r.norm,std::abs(double(bf16_value(y[n]))-bf16_value(expected))/std::max(ab,1e-30));}return r;}
}

int main(int argc,char**argv){try{std::string output;for(int i=1;i<argc;++i){std::string_view f=argv[i];if(f=="--output"&&++i<argc)output=argv[i];else throw std::runtime_error("usage: --output FILE");}check(cudaSetDevice(0),"device");cudaDeviceProp prop{};check(cudaGetDeviceProperties(&prop,0),"properties");if(prop.major!=8||prop.minor!=6)throw std::runtime_error("requires SM86");std::uint32_t state=0x514a9c31U;HostMatrix qa(1024,4096,state),qb(40960,1024,state),kv(512,4096,state),oa(8192,4096,state),ob(4096,8192,state);DevMatrix dqa(qa,16),dqb(qb,1),dkv(kv,16),doa(oa,4),dob(ob,8);std::vector<std::uint16_t>input(4096),attention(4096);for(auto&v:input)v=bf16_bits(float(int(rng(state)%17)-8)/8.0F);for(auto&v:attention)v=bf16_bits(float(int(rng(state)%17)-8)/8.0F);Buffer din(input.size()*2),datt(attention.size()*2),barrier(8),scrub(kScrubBytes),ruler(kRulerBytes),sink(1024);const std::size_t peak=dqa.bytes()+dqb.bytes()+dkv.bytes()+doa.bytes()+dob.bytes()+din.bytes()+datt.bytes()+barrier.bytes()+scrub.bytes()+ruler.bytes()+sink.bytes();if(peak>kCeiling)throw std::runtime_error("exceeds 512 MiB");cudaStream_t st;check(cudaStreamCreateWithFlags(&st,cudaStreamNonBlocking),"stream");auto upload=[&](const HostMatrix&h,DevMatrix&d){check(cudaMemcpyAsync(d.code.get(),h.packed.data(),d.code.bytes(),cudaMemcpyHostToDevice,st),"code");check(cudaMemcpyAsync(d.scale.get(),h.scales.data(),d.scale.bytes(),cudaMemcpyHostToDevice,st),"scale");check(cudaMemsetAsync(d.counter.get(),0,d.counter.bytes(),st),"counter");};upload(qa,dqa);upload(qb,dqb);upload(kv,dkv);upload(oa,doa);upload(ob,dob);check(cudaMemcpyAsync(din.get(),input.data(),din.bytes(),cudaMemcpyHostToDevice,st),"input");check(cudaMemcpyAsync(datt.get(),attention.data(),datt.bytes(),cudaMemcpyHostToDevice,st),"attention");check(cudaMemsetAsync(barrier.get(),0,barrier.bytes(),st),"barrier");check(cudaMemsetAsync(ruler.get(),0xa5,ruler.bytes(),st),"ruler");check(cudaStreamSynchronize(st),"uploads");const auto blocks=prop.multiProcessorCount*kResidentBlocksPerSm;auto run=[&]{scheduler_kernel<<<blocks,kThreads,0,st>>>((const uint4*)dqa.code.get(),(const std::uint8_t*)dqa.scale.get(),(const uint4*)dqb.code.get(),(const std::uint8_t*)dqb.scale.get(),(const uint4*)dkv.code.get(),(const std::uint8_t*)dkv.scale.get(),(const uint4*)doa.code.get(),(const std::uint8_t*)doa.scale.get(),(const uint4*)dob.code.get(),(const std::uint8_t*)dob.scale.get(),(const std::uint16_t*)din.get(),(const std::uint16_t*)datt.get(),(float*)dqa.partial.get(),(float*)dqb.partial.get(),(float*)dkv.partial.get(),(float*)doa.partial.get(),(float*)dob.partial.get(),(std::uint32_t*)dqa.counter.get(),(std::uint32_t*)dqb.counter.get(),(std::uint32_t*)dkv.counter.get(),(std::uint32_t*)doa.counter.get(),(std::uint32_t*)dob.counter.get(),(std::uint16_t*)dqa.out.get(),(std::uint16_t*)dqb.out.get(),(std::uint16_t*)dkv.out.get(),(std::uint16_t*)doa.out.get(),(std::uint16_t*)dob.out.get(),(std::uint32_t*)barrier.get());};auto doscrub=[&]{scrub_kernel<<<65536,256,0,st>>>((std::uint8_t*)scrub.get(),scrub.bytes());};auto doruler=[&]{ruler_kernel<<<prop.multiProcessorCount*8,256,0,st>>>((const uint4*)ruler.get(),ruler.bytes()/16,(unsigned*)sink.get());};for(unsigned i=0;i<kWarmups;++i){doruler();run();}check(cudaStreamSynchronize(st),"warmups");cudaEvent_t a,b;check(cudaEventCreate(&a),"event");check(cudaEventCreate(&b),"event");std::vector<float>rt,kt;for(unsigned i=0;i<kSamples;++i){doscrub();rt.push_back(timed(a,b,st,doruler));doscrub();kt.push_back(timed(a,b,st,run));}check(cudaGetLastError(),"scheduler");auto download=[&](DevMatrix&d,const HostMatrix&h){std::vector<std::uint16_t>v(h.n);check(cudaMemcpy(v.data(),d.out.get(),d.out.bytes(),cudaMemcpyDeviceToHost),"output");return v;};auto yqa=download(dqa,qa),yqb=download(dqb,qb),ykv=download(dkv,kv),yoa=download(doa,oa),yob=download(dob,ob);const auto eqa=oracle(qa,input,yqa),eqb=oracle(qb,yqa,yqb),ekv=oracle(kv,input,ykv),eoa=oracle(oa,attention,yoa),eob=oracle(ob,yoa,yob);const float ru=median(rt),ku=median(kt);const std::size_t useful=qa.bytes()+qb.bytes()+kv.bytes()+oa.bytes()+ob.bytes();const double rgb=double(kRulerBytes)/ru/1000.0,gb=double(useful)/ku/1000.0;std::ostream*os=&std::cout;std::ofstream file;if(!output.empty()){file.open(output);os=&file;}*os<<std::fixed<<std::setprecision(9)<<"{\n  \"device_name\": \""<<prop.name<<"\", \"resident_blocks\": "<<blocks<<",\n  \"useful_bytes\": "<<useful<<", \"peak_device_bytes\": "<<peak<<",\n  \"ruler_us\": "<<ru<<", \"ruler_gbps\": "<<rgb<<",\n  \"scheduler_us\": "<<ku<<", \"effective_gbps\": "<<gb<<", \"roofline_efficiency\": "<<(gb/rgb)<<",\n  \"q_a_mismatches\": "<<eqa.bad<<", \"q_a_maximum_error_over_sum_abs\": "<<eqa.norm<<",\n  \"q_b_indexer_mismatches\": "<<eqb.bad<<", \"q_b_indexer_maximum_error_over_sum_abs\": "<<eqb.norm<<",\n  \"wkv_mismatches\": "<<ekv.bad<<", \"wkv_maximum_error_over_sum_abs\": "<<ekv.norm<<",\n  \"wo_a_mismatches\": "<<eoa.bad<<", \"wo_a_maximum_error_over_sum_abs\": "<<eoa.norm<<",\n  \"wo_b_mismatches\": "<<eob.bad<<", \"wo_b_maximum_error_over_sum_abs\": "<<eob.norm<<"\n}\n";cudaEventDestroy(a);cudaEventDestroy(b);cudaStreamDestroy(st);return EXIT_SUCCESS;}catch(const std::exception&e){std::cerr<<"error: "<<e.what()<<'\n';return EXIT_FAILURE;}}
