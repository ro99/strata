...
| F4-3 | Clear FP4 surpass curve | >=632 GB/s at M `{1,4,8}` both shapes and >301.9 GB/s at M=16 | **COMPLETE** (experiment 0143) | 742.9/749.9, 731.4/736.2, 704.1/666.7, and 523.2/478.9 at M=16; 88-83% of the read floor at M<=8 |
...
| F8-1 | Prove exact QPN8-derived FP8 primitive | E4M3/E8M0 block-128 fragment prepack and direct W8A16 register feed; independent decoder/matrix/boundary gates; no widened persistent path | **IN PROGRESS on owner-authorized Ampere-native persistent/fused successor (experiments 0145--0146); experiment 0144's per-projection rejection stands** | Full compact W8A16 kernel plus layer-resident scheduler now reach 699.67 GB/s / 83.27% over real projection shapes and dependencies. Real-fixture/no-worse numerical closure remains open before the M curve |
| F8-2 | Clear owner-bound FP8 performance curve | Satisfy D-F8-GATE on every required shape/M with three interleaved process medians and independent correctness | **BLOCKED on completion of the active F8-1 successor** | Equal-local-roofline gate remains binding and unmoved. No M curve or production persistent dispatch exists yet |
...
- **Current milestones:** C2 and F4-1 are complete. F4-2 is cleared at eight or
  ...
  complete. F8-1's exact primitive phase passes. Experiment 0144's
  per-projection launch architecture remains rejected; after explicit owner
  authorization, experiment 0145 opened an Ampere-native persistent/fused
  successor and built the first full compact W8A16 kernel. F8-2 remains
  blocked until that successor closes its scheduler and numerical gates.
...
- **THE FP4 TRACK HAS CLEARED EVERY PERFORMANCE GATE: F4-1, F4-2, F4-3.**
  Experiment 0143's surpass curve at 32 experts, split-K 2: **742.9/749.9 GB/s
...
- **Two tuning rules the campaign paid for.** A **runtime bound on a short inner
  loop** stopped full unrolling and cost 24% at M=1 (0143). **Split-K partial
...
**The FP4 track has cleared every performance gate: F4-1, F4-2 and F4-3**
(experiments 0140, 0142, 0143). Next, in order:
...
2. **Add the E8M0 0/255 admission check**, owed since 0137. Both decoders are
   wrong on those two codes.
3. **Begin MIX-1** only after (1): one-copy mixed production dispatch, route
   census, admission, load-time prepack cost, VRAM accounting, graph
   integration, fixtures.
4. **F8-0 is open, unblocked, and independent** — inventory every real FP8
   region, operation, `(M,N,K)`, activation carrier, publication boundary and
   frequency, then measure the incumbent, the W8A8-style control, the read
   ceiling, exact bytes and `argmax` per M band.

**D-F8-GATE remains an open owner decision** and still blocks any FP8
performance claim.
...
**It is FP4, not NVFP4.** NVFP4 is E2M1 with FP8 E4M3 group-16 scales; Strata's
format, which this kernel decodes exactly, is E2M1 with E8M0 group-32 scales.
No agent may write "NVFP4" for this work. And no end-to-end claim exists until
MIX-2: kernel bandwidth is not serving throughput.
