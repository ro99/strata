...
- **REAL WEIGHTS: bit-exact (0156).** The kernel runs on production bytes from
  ...
  the `F8_E4M3` tensors are attention, not this track. **Still owed:**
  multi-layer/multi-expert fixtures, real activations, and the operation/layer
  plus full-runtime oracles.
...
Remaining FP4 validation before MIX-1, none of it blocking on FP8:

1. **Widen the numerical fixture** across layers and expert indices; one expert
   in one layer cannot expose a layout assumption that varies with layer.
2. **Capture real activations at the expert boundary**, so both operands are
   production data rather than only the weights.
