...
| MIX-1 | Integrate one-copy mixed production dispatch | Eligible FP4 uses accepted F4 path; eligible FP8 uses accepted F8 path; unsupported shapes use explicit approved exact routes; route census, admission, prepack, VRAM, graph and fixtures pass | **IN PROGRESS (0160)** — route census and explicit unsupported route done; accepted-path substitution, admission wiring, prepack, VRAM, graph and fixtures outstanding | No hidden fallback, no duplicate/widened weights |
