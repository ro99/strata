...
| MIX-1 | Integrate one-copy mixed production dispatch | Eligible FP4 uses accepted F4 path; eligible FP8 uses accepted F8 path; unsupported shapes use explicit approved exact routes; route census, admission, prepack, VRAM, graph and fixtures pass | **IN PROGRESS (0160-0162)** — admission done and verified on the real 156 GB checkpoint; census done for `matmul_impl`, which 0162 showed is **load-only**. Census of the real decode paths (`enqueue_moe`, rank-local executors, attention chain), accepted-path substitution there, prepack, VRAM, graph and fixtures outstanding | No hidden fallback, no duplicate/widened weights |
