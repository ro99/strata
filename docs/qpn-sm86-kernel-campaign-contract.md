...
- **Open FP4 limitations:** the group-32 scale-to-K binding is unproven and no
  probe has crossed a group boundary in fragment order; the successor's
  16-bit-apart nibble pairing is a prepack choice that must be re-derived
  against the real fragment layout; E8M0 codes 0 and 255 are wrong in both
  decoders and need admission; and no M curve exists.
