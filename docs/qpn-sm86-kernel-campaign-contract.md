...
- **The candidate is 3.2x short of its own ceiling, and `argmax` is measured:
  load granularity.** It reads 4 bytes per lane per K-tile where the ceiling
  probe reads 16, paying 4x the loads and loop control per useful byte.
  Falsified as causes, by measurement: parallelism, the scale-load pattern,
  activation divergence, and the MMA accumulator chain — a `--no-mma` arm
  measures identical.
- **Open FP4 limitations:** E8M0 codes 0 and 255 are wrong in both decoders and
  need admission; no M curve exists; and the candidate's load granularity is
  unfixed.
...
3b. **Raise the load granularity to a `uint4` per lane** — this is 0140's
   measured `argmax` and the only remaining term between the candidate and its
   826 GB/s ceiling. Restack the prepack so a lane's four consecutive K-tile
   words are contiguous, `((n_tile*(k_tiles/4) + kb)*32 + lane)*4 + j`, so one
   16-byte load feeds four MMAs and consecutive lanes read consecutive 16-byte
   chunks. Only if that does not close the gap, consider multiple N-tiles per
   warp sharing one B fragment (thesis item 5), and only on measured evidence.
