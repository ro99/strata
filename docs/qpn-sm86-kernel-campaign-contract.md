...
- **The candidate is at 427.6/449.2 GB/s steady state (0141), 1.93x short of
  its own 826 GB/s ceiling and short of the 600.0 gate.** 0140's attribution to
  load granularity **over-claimed**: the `uint4` fix was worth 13-22%, not the
  predicted 3-4x. Four terms were comparable, not one dominant. Falsified as
  causes by measurement: parallelism, scale-load pattern, activation
  divergence, MMA accumulator chain, load granularity, partial traffic, and
  reduction structure. **The residual is un-attributed — most likely per-warp
  prologue against a short trip count — and no further optimisation should be
  attempted until an Nsight profile attributes it. Guessing has been wrong
  twice.**
...
1. ~~Re-measure whether the MMA is still free.~~ **DONE, experiment 0139: the
   MMA costs 0.63%/0.64% and is still free. The candidate keeps 27–28% of the
   gate's time budget — about 2 µs per matrix pass — for everything below.**
2. ~~Build the fragment prepack and prove the scale-to-K binding.~~ **DONE,
   experiment 0140: bit-exact over 128 and 64 K-group boundaries, prepack is a
   pure permutation, and the derivation caught a real scale defect in 0137.**
   Superseded text: build the E2M1/E8M0 group-32 fragment prepack for
   ...
   that pairing is itself a prepack choice, so it must be re-derived against the
   real fragment layout rather than assumed to survive.
3. **Add an admission check for E8M0 codes 0 and 255.**
3b. ~~Raise the load granularity to a `uint4` per lane.~~ **DONE, experiment
   0141: worth 13-22%, not the predicted 3-4x. Three further optimisations
   applied, cumulative 1.67x to 427.6/449.2 GB/s steady state.**
3c. **Resolve the measurement footing with the owner**, then **attribute the
   residual 1.93x with an Nsight profile before optimising further** — warp-state
   sampling, issue efficiency and memory throughput at split-K 8. Two guesses
   have been wrong; the next step must be a profile, not a fourth hypothesis.
4. **Time a full candidate step** — prepack, activation feed, MMA, output and
   ...
   **sustained SM clock and the run spread** alongside the median, so neither
   the measured 9.4% cold-protocol inflation nor the 8–13% boost variance can
   hide inside the result.
