...
| MIX-1 | Integrate one-copy mixed production dispatch | Eligible FP4 uses accepted F4 path; eligible FP8 uses accepted F8 path; unsupported shapes use explicit approved exact routes; route census, admission, prepack, VRAM, graph and fixtures pass | **IN PROGRESS (0160-0163)** — admission done and verified on the real checkpoint; census done and the decode route map settled (0163). **Blocked on an owner decision for the FP4 half**: routed FP4 experts run on the host and the only per-token CUDA FP4 site is the opt-in resident tier. F8 substitution into the shared expert needs no decision. Prepack, VRAM, graph and fixtures outstanding | No hidden fallback, no duplicate/widened weights |
