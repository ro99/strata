...
| MIX-1 | Integrate one-copy mixed production dispatch | Eligible FP4 uses accepted F4 path; eligible FP8 uses accepted F8 path; unsupported shapes use explicit approved exact routes; route census, admission, prepack, VRAM, graph and fixtures pass | **SUBSTITUTION DONE, VALUE UNMEASURED (0160-0163, then the wiring at `180a050`)** — admission, census and the decode route map are done; the register-fed FP4 and FP8 routes are now the default in `matmul_impl` for every model and in both DeepSeek shared-expert dispatch sites, with lazy in-place prepack, a runtime A/B switch and route-against-route equivalence proven at M `{1,5,8,16}`. **Still blocked on an owner decision for the FP4 half**: routed FP4 experts run on the host and the only per-token CUDA FP4 site is the opt-in resident tier. VRAM, graph and fixture checks outstanding, and no end-to-end run has been made | No hidden fallback, no duplicate/widened weights. The scalar routes are kept deliberately: for extents the fragment layout cannot express, for grouped matmul, and as the A/B control the evidence rules require |
| MIX-2 | Confirm end-to-end value | Real workload shows material outside-variance improvement with identical model, formats, activations, routes, and budgets; phase/resource traffic reported | **MEASURED NEGATIVE for DeepSeek V4 decode** | Decode `argmax_r` is the host MoE wait at 129.77 ms of a 167.4 ms step (78%). All CUDA kernel time is about 3.4 ms, so the ceiling on any kernel substitution is 1.02x; measured 167.4 regfed against 164.0 scalar. Kernel bandwidth was never an end-to-end claim and is not one here |
