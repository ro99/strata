...
| MIX-1 | Integrate one-copy mixed production dispatch | Eligible FP4 uses accepted F4 path; eligible FP8 uses accepted F8 path; unsupported shapes use explicit approved exact routes; route census, admission, prepack, VRAM, graph and fixtures pass | **PROVEN ON GEMMA 4 MXFP4; DEEPSEEK OPT-IN REMAINS DORMANT (0160-0165)** — fragment prepack is explicit, never inferred by `matmul_impl`. Gemma's loader prepackages only MXFP4 after both consumers were audited and converted; W8A16 remains canonical. Route-vs-route tests cover real Gemma projection shapes, the scalar full-model oracle is coherent, the format-specific 1,080-layer-hash oracle passes, and every canonical consumer refuses fragment order. DeepSeek remains deliberately unopted because its other consumers and cost model make conversion negative | No hidden fallback, no duplicate/widened weights. The scalar routes remain the A/B/control and serve canonical W8A16 unchanged |
| MIX-2 | Confirm end-to-end value | Real workload shows material outside-variance improvement with identical model, formats, activations, routes, and budgets; phase/resource traffic reported | **POSITIVE for Gemma 4 MXFP4 decode; MEASURED NEGATIVE for DeepSeek V4 decode** | Gemma scalar profiling found GPU kernel/HBM service at 184.984 ms of a 186.708 ms steady step (99.1%), so the mechanism attacks `argmax_r`. Three interleaved reps at the production 250 W / 1605 MHz point measured 55.467 ms/token register-fed versus 186.757 scalar, **3.367x**, with 0.212/0.111 ms spreads, identical 32-token streams in all six arms, and valid nonzero censuses. DeepSeek remains negative: host MoE wait is 78% and caps the kernel substitution at 1.02x |
