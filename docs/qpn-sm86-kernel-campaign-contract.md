...
| F8-2 | Clear owner-bound FP8 performance curve | Satisfy D-F8-GATE on every required shape/M with three interleaved process medians and independent correctness | **M=1 PASSES (0158); M `{2,3,4,8,16}` UNMEASURED** | Equal-local-roofline gate remains binding and unmoved. The five-CTA M=1 upper bound passes with 5.633 us margin, but it omits experiment 0147's sparse projection replay. M `{2,3,4,8,16}` remains unmeasured; no production dispatch exists |
