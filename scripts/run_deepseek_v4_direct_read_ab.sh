...
run_one() {
    ...
    local repetition=$2
    local binary=$3
    local stem="${result_dir}/${repetition}-${variant}"
...
run_one candidate pair "${candidate}"
run_one reference pair "${reference}"
...
jq -n \
    ...
    {
        candidate: ($candidate[0] | fields),
        reference: ($reference[0] | fields),
