...
result_dir=${RESULT_DIR:-"${repo_root}/results/deepseek-v4-overlap-stage-warmup-ab"}
candidate=${CANDIDATE_BINARY:-"${repo_root}/build/strata-deepseek-run"}
reference=${REFERENCE_BINARY:-"/tmp/strata-dsv4-overlap-stage-warmup-baseline/build/strata-deepseek-run"}
...
run_one candidate "${candidate}" --overlap-resident-warmup
...
jq -n \
    ...
    --slurpfile reference "${result_dir}/pair-reference.json" '
    {
        ...
        cache_hits_match: ($candidate[0].vram_cache_hits ==
                           $reference[0].vram_cache_hits),
        cache_misses_match: ($candidate[0].vram_cache_misses ==
                             $reference[0].vram_cache_misses),
        cache_evictions_match: ($candidate[0].vram_cache_evictions ==
                                $reference[0].vram_cache_evictions)
    }
