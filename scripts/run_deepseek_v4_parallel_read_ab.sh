...
result_dir=${RESULT_DIR:-"${repo_root}/results/deepseek-v4-parallel-read-ab"}
candidate=${CANDIDATE_BINARY:-"${repo_root}/build/strata-deepseek-run"}
reference=${REFERENCE_BINARY:-"/tmp/strata-dsv4-parallel-read-baseline/build/strata-deepseek-run"}
read_workers=${RESIDENT_READ_WORKERS:-8}
...
run_one candidate "${candidate}" --resident-read-workers "${read_workers}"
...
jq -n \
    ...
    --slurpfile reference "${result_dir}/pair-reference.json" '
    {
        ...
        staging_speedup: ($reference[0].resident_staging_seconds /
